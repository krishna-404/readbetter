...
function BooksController(){
    ...
    this.newBook = async function(req, res){
        ...
        let leader = await LeaderModel.findOne({'twitter.id': req.body.twitter_id}).lean();

        //return error if leader not already added
        if(!leader) return res.send("Invalid twitter id");
        
        //check if the book exists
        let book = await BookModel.findOne({$or : [{ISBN13: req.body.ISBN}, 
                                                    {$and : [{book_name: req.body.book_name},
                                                            {book_author : req.body.author}]}]}).lean();
        //if book doesn't exist, add book & update in the leader DB also                                                    
        if(!book){
            
            LeaderModel.findOneAndUpdate(   {'twitter.id': req.body.twitter_id}, 
                                            {$set:{updated_on : new Date(), updated_by: req.connection.remoteAddress}, 
            ...
            const bookInLeaderModel = leader.booksReco.some( e => {
                const byName = e.book_name == req.body.book_name && e.book_author == req.body.author;
            ...
            // by default parallel entry should be available in both database
                ...
                await LeaderModel.updateOne({'twitter.id': req.body.twitter_id},{
                                                $set:{updated_on : new Date(), updated_by: req.connection.remoteAddress}, 
                                                $push:{booksReco: booksReco}
                                            });
                ...
                await BookModel.updateOne({_id: book._id},{
                                                 $set:{updated_on : new Date(), updated_by: req.connection.remoteAddress}, 
